import { publicUser, rowToVideo, scoreVideo, sqlite } from "./sqliteStore.js";
...
function videoScore(row) {
  return number(row.likes) + number(row.comments) * 2 + number(row.shares) * 4 + number(row.gifts) * 5;
}
...
function creatorSummary(userId) {
  ...
  return {
    ...
    gifts,
    score: number(row.likes) + number(row.comments) * 2 + number(row.shares) * 4 + gifts * 5,
    estimatedEarnings,
...
function topCreatorVideos(userId) {
  return safeAll("SELECT * FROM videos WHERE user_id = ?", [userId])
    ...
    .slice(0, 8)
    .map((row) => ({ ...rowToVideo(row, null), performanceScore: videoScore(row) }));
}
...
export function registerAnalyticsRoutes(app, getAuthUser) {
  ...
  app.get("/api/admin/analytics", requireAuth, requireAdmin, (_req, res) => {
    ...
    `).map((row) => ({
      ...
      score: number(row.likes) + number(row.comments) * 2 + number(row.shares) * 4 + number(row.gifts) * 5
