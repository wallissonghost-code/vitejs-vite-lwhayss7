...
import { createVideoUploader } from "./uploadStorage.js";
...
const { upload, uploadsDir, publicUrlForFile, uploadConfig } = createVideoUploader(ROOT_DIR);
...
app.post("/api/videos", requireAuth, upload.single("video"), (req, res) => {
  ...
  const uploadedUrl = publicUrlForFile(req.file);
  const videoUrl = uploadedUrl || String(req.body.videoUrl || "").trim() || sampleVideos[0];
  const tags = toJson((caption.match(/#[a-zA-Z0-9_À-ÿ]+/g) || ["#gxst", "#vibes"]).map((tag) => tag.replace("#", "").toLowerCase()));
...
app.use((err, _req, res, _next) => {
  ...
  const status = message.includes("arquivo") || message.includes("vídeo") || message.includes("Upload") ? 400 : 500;
...
app.listen(PORT, () => console.log(`GXST Vibes V12 em http://localhost:${PORT}`));
